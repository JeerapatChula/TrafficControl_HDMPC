...
class Model2():
    ...
    def __init__(self, intersection, intersection_configs, model_incoming_roads, model_outcome_roads, controller_config):
        ...
        self.roads_capacity = self.get_road_capacities()
        ...
        for i in range(self.num_phase):
            self.log_predict_x[f"p{i}"] = []
            self.log_predict_y[f"p{i}"] = []

    def predict_state(self, input_u):
        # return
        self.state = self.get_state()

        self.log_x.append(self.state)
        self.log_y.append(np.matmul(self.matrix_c, self.state))

        for i in range(self.num_phase):
            x_hat_temp, y_hat_temp = self.model_evaluate(self.state, input_u, i)
            self.log_predict_x[f"p{i}"].append(x_hat_temp)
            self.log_predict_y[f"p{i}"].append(y_hat_temp)
        # exit((self.state, x_hat_temp))
    ...
    def get_road_capacities(self):
        road_capacities = None

        for road in self.model_incoming_roads:
            road_capacities = road.capacity if road_capacities is None else np.vstack((road_capacities, road.capacity))

        for road in self.model_outcome_roads:
            road_capacities = np.vstack((road_capacities, road.capacity))
    ...
    def get_input_to_phase_matrix(self, phase):
        temp = np.zeros((self.num_phase, self.num_phase))
        temp[phase, phase] = 1
        return temp
    ...
    @staticmethod
    def func_xhat_l3(a, b, sum_a, roads_capacity):
        if sum_a <= roads_capacity:
            return a
        else:
            return (a/sum_a)*b

    def model_evaluate(self, state, input_u, phase):
        input_to_phase_matrix = self.get_input_to_phase_matrix(phase)
        vfunc_max = np.vectorize(self.func_max)
        vfunc_min = np.vectorize(self.func_min)
        vfunc_xhat_l3 = np.vectorize(self.func_xhat_l3)

        # ---------------- Layer 1 ---------------- #
        exogenous = np.matmul(self.matrix_p, self.predicted_flow*input_u[phase])

        # print(state)
        # print(self.matrix_b[:, phase])
        # print(exogenous)

        temp_a = state + np.matmul(self.matrix_b , np.matmul(input_to_phase_matrix, input_u.reshape((-1, 1)))) + exogenous
        # exit(temp_a)
        temp_a = vfunc_max(temp_a, 0)

        # ---------------- Layer 2 ---------------- #
        sum_a = np.matmul(self.matrix_c, temp_a)
        temp_b = vfunc_max(sum_a, 0)
        # exit((sum_a,self.roads_capacitis))
        temp_b = vfunc_min(temp_b, self.roads_capacity)

        # ---------------- Layer 3 ---------------- #
        # exit(np.matmul(self.matrix_c.T, self.roads_capacity))
        x_hat_temp = vfunc_xhat_l3(temp_a, np.matmul(self.matrix_c.T, temp_b), np.matmul(self.matrix_c.T, sum_a), np.matmul(self.matrix_c.T, self.roads_capacity))

        # ---------------- Layer 3 ---------------- #
        y_hat_temp = np.matmul(self.matrix_c, x_hat_temp)

        return x_hat_temp, y_hat_temp

    def cost_1(self, u):
        E = None

        for i in range(self.num_phase):
            x_hat_temp, y_hat_temp = self.model_evaluate(self.state, u, i)
            E = y_hat_temp if E is None else np.vstack((E, y_hat_temp))

        return E, (LA.norm(E, 2)**2)/2

    def cost_2(self, u):
        q = None
        Y_old = np.matmul(self.matrix_c, self.state)

        for i in range(self.num_phase):
            x_hat_temp, y_hat_temp = self.model_evaluate(self.state, u, i)
            q = q + (Y_old - y_hat_temp) if q is not None else Y_old - y_hat_temp
            Y_old = y_hat_temp
    ...
    @staticmethod
    def func_xhat_l3_dar(a, b, sum_a, roads_capacity):
        if sum_a <= roads_capacity:
            return [1, 0]
        else:
            return [a/sum_a, b]
    ...
    def get_control_signal(self):
        ...
        self.output_log.append(np.matmul(self.matrix_c, self.state))
        ...
        res = minimize(self.func, initial, method='SLSQP', constraints=[eq_cons], options={'disp': False}, bounds=self.control_bounds)
    ...
    def save(self, name, output_path):
        filename = "{}/controllers/intersections_testmodel/{}.mat".format(output_path, name)
        data = {
            "log_x": self.log_x,
            "log_y": self.log_y,
            "log_predict_x": self.log_predict_x,
            "log_predict_y": self.log_predict_y,
...
class Intersection():
    ...
    def __init__(self, intermediary, name, controller_config, network_config):
        ...
        self.area_roads = Agreement.get_area_road_list(self.my_area, network_config)
        ...
        self.model_incoming_roads = [Road(road, self.intersection_configs["incoming_roads"][road], network_config.roads[road], True, available_lanes, incoming_roads) for road in self.incoming_roads]
        self.model_outcome_roads = [Road(road, self.intersection_configs["outcome_roads"][road], network_config.roads[road], False, available_lanes, incoming_roads) for road in self.outcome_roads]
        ...
        self.model2 = Model2(name, self.intersection_configs, self.model_incoming_roads, self.model_outcome_roads, controller_config)
    ...
    def upper_data_receiver(self, data):
        ...
        self.model2.set_ref_flow(self.ref_signal)
    ...
    def create_distribution_matrix(self):
        num_area_roads = len(self.area_roads)
        num_my_links = len(self.roads)
        matrix = np.zeros((num_my_links, num_area_roads))
        ...
        for (road, dst) in zip(self.roads, range(num_my_links)):
            ...
            src = self.area_roads.index(road)
        ...
        self.distribution_matrix = (1/self.area_rate)*matrix
    ...
    def cycle_completed_signal(self, data):
        ...
        if self.is_controlled:
            ...
            self.model2.put_predicted_flow(predicted_flow)
            control_signal = self.model2.get_control_signal()
            # self.model2.predict_state(control_signal)
    ...
    def save(self, output_path):
        ...
        self.model2.save(self.name, output_path)
