...
class Agreement():
    ...
    def get_network_node_list(network_config):
        nodes = list(network_config.areas.keys())
        return sorted(nodes)
    ...
    def get_adjacend_areas(area, network_config):
        ...
        area_nodes = Agreement.get_area_node_list(area, network_config)
        adjacend_areas = []
        for (link, src) in zip(network_links, range(len(network_links))):
            if network_config.links[link]["source_node"] in area_nodes:
                adjacend_areas.append(network_config.links[link]["source_node"])
            elif network_config.links[link]["sink_node"] in area_nodes:
                adjacend_areas.append(network_config.links[link]["sink_node"])
        return list(set(adjacend_areas))
    ...
    def get_area_node_list(area, network_config):
        ...
        for intersection in intersection_list:
            ...
            for road in incoming_roads:
                nodes.append(incoming_roads[road]["boundary"])
            for road in outcome_roads:
                nodes.append(outcome_roads[road]["boundary"])
