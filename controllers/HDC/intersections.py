...
class Model():
    ...
    def __init__(self, intersection_configs, model_incoming_roads, model_outcome_roads, controller_config):
    ...
    def func(self, u):
        ## This function is evaluates the cost J = || C(x(t) + Bu(t) + w(t)) ||^2 + y|| q*(t) - q^(t)||^2, eq(42). ##
        CB = np.matmul(self.matrix_c, np.absolute(self.matrix_b))/self.cycle_length
        q = np.matmul(CB, u.reshape((-1, 1)))
        exogenous = self.predicted_flow*self.cycle_length

        E1 = self.mean_state + np.matmul(self.matrix_b, u.reshape((-1, 1))) + np.matmul(self.matrix_p, exogenous)
        E2 = q - self.ref
        E1 = (LA.norm(E1, 2)**2)/2
        E2 = (LA.norm(E2, 2)**2)/2
    ...
    def func_dar(self, u):
        ...
        exogenous = self.predicted_flow*self.cycle_length
        E1 = self.mean_state + np.matmul(self.matrix_b, u.reshape((-1, 1))) + np.matmul(self.matrix_p, exogenous)
        E1 = np.matmul(E1.T, self.matrix_b)
        E2 = q - self.ref
        E2 = np.matmul(E2.T, CB)
        return np.squeeze(np.asarray(E1 + self.gamma*E2))

    def create_con(self):
        self.matrix_con_eq = np.ones((1, self.num_phase))
        self.vector_con_eq = self.cycle_length
    ...
    def get_control_signal(self):
        ...
        self.mean_state_log.append(self.mean_state)
        self.ref_log.append(self.ref)
        self.predicted_flow_log.append(self.predicted_flow)

        eq_cons = {'type': 'eq',
                    'fun' : lambda x: np.squeeze(np.matmul(self.matrix_con_eq, x.reshape((-1, 1))) - self.vector_con_eq),
                    'jac' : lambda x:self.matrix_con_eq}

        initial = np.zeros((self.num_phase, 1))
        ...
        control_signal = (res.x).astype(int)
...
class Intersection():
    ...
    def __init__(self, intermediary, name, controller_config, network_config):
        ...
        self.model = Model(self.intersection_configs, self.model_incoming_roads, self.model_outcome_roads, controller_config)
    ...
    def cycle_completed_signal(self, data):
        ...
        if self.is_controlled:
            ...
            # print("{} - {}".format(self.name, control_signal))
        else:
            self.old_signal = data["old_signal"]
